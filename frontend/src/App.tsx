...
function TabBar({ route }: { route: Route }) {
  ...
  // The FAB now opens a chooser rather than going straight to New Pour Card:
  // there are two things worth creating from the field, and burying either one
  // in a menu the other doesn't use would be worse than one extra tap.
  const go = (next: Route) => {
    setMenuOpen(false);
    navigate(next);
  };
